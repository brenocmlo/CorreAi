...
export default function ChatbotWidget({ context }: ChatbotWidgetProps) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (context === "cliente" && storedLeadName) {
        setMessages([
          {
            id: 1,
            text: `Olá, ${storedLeadName}! Sou o assistente virtual CorretAI do seu corretor. Como posso ajudar você a encontrar seu imóvel hoje?`,
            sender: "bot",
          }
        ]);
      }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // 2. Se for cliente e já tiver cadastro, usa a API de webhook real do backend Express (com dados do banco)
      if (context === "cliente" && leadId && brokerId) {
        ...
        // Caso contrário (corretor ou cliente sem cadastro), usa a API genérica do Next.js
